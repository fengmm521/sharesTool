...
#提交数据到MySQL数据库
class MySqlDataTool():
    def __init__(self,addr = 'woodcol.com',port = 3306,usname = 'tfuser',uspw = 'GuPiao1qaz',defDB = 'shares_dat'):#root,7668150My00
...
def updateDataToSql():
    ...
    for d in ids:
        ...
        tmpid = '300208'
        if tmpid in tmpsaves:
            print 'find:' + str(tmpid)
            continue
        f = open('addid.txt','a')
        strtmpx = str(tmpid) + '\n'
        f.write(strtmpx)
        f.close()
        csvdatas,cntype = AnalysisData(d)   #读取数据和数据中的中文编码格式
        sqlback = sqltool.saveCSVDataToSql(tmpid, csvdatas,id1s[tmpid][1])
        if sqlback > 100:
            print sqlback
            print 'erro ID:' + str(tmpid)
            return
        print str(tmpid) + '编号股票所有数据据上传完成'
        return
        time.sleep(0.001)
#测试
if __name__ == '__main__':
    # sqltool = MySqlDataTool()
    # sqltool.createOneTab('000001')
    updateDataToSql()
